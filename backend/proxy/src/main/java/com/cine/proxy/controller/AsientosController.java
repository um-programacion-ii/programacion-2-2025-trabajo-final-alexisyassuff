...
public class AsientosController {
    ...
    public ResponseEntity<List<Map<String,Object>>> getAsientos(
        ...
        try {
            ...
            // 1. Obtener dimensiones desde cátedra
            ...
            // 2. Matriz base (todo libre)
            List<Map<String,Object>> allSeats = generateBaseMatrix(filas, columnas);

            // 3. Mergeo de estados actuales desde Redis (incluye vendido, bloqueado, etc)
            mergeRedisStates(allSeats, eventoId, sessionId);

            // 4. Fallback para legacy (sold/lock keys)
            mergeFallbackLegacy(allSeats, eventoId, sessionId);

            // 5. Mergea memoria local si falta info
            mergeLocalMemory(allSeats, eventoId, sessionId);
                    ...
                    for (String name : names) {
                        ...
                        if (!n.isMissingNode() && !n.isNull()) {
                            ...
                            if (txt != null && !txt.isBlank()) {
                                try { return Integer.parseInt(txt.trim()); } catch (NumberFormatException ex) {
                                    try {
    ...
    private void mergeFallbackLegacy(List<Map<String, Object>> allSeats, String eventoId, String sessionId) {
        long mergedCount = 0;
        // sold keys
        try {
            ...
            log.error("Error sold legacy merge: {}", e.getMessage());
        }
        ...
        try {
            ...
            log.error("Error lock legacy merge: {}", e.getMessage());
        }
    }

    private void mergeLocalMemory(List<Map<String, Object>> allSeats, String eventoIdStr, String sessionId) {
        int eventoId;
        try {
    ...
    @PostMapping("/api/endpoints/v1/desbloquear-asiento")
    ...
    @PostMapping("/api/endpoints/v1/desbloquear-asientos")

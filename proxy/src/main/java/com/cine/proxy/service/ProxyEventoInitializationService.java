...
/**
 * EventoInitializationService para el PROXY - Inicializa eventos en Redis desde datos de Cátedra.
 * 
 * Responsabilidades:
 ...
 */
@Service
public class ProxyEventoInitializationService {

    private static final Logger log = LoggerFactory.getLogger(ProxyEventoInitializationService.class);
    ...
    public ProxyEventoInitializationService(StringRedisTemplate redis, 
                                          ObjectMapper mapper,
    ...
    public boolean ensureEventoInitialized(String eventoId) {
        ...
        try {
            // 1. Verificar si ya existe en Redis (primero como JSON string)
            String existingData = redis.opsForValue().get(redisKey);
            if (existingData != null && !existingData.trim().isEmpty()) {
                log.debug("Evento {} ya existe en Redis como JSON string, saltando inicialización", eventoId);
                return true;
            }
            
            // 2. Verificar si existe como hash (formato legacy)
            Boolean hashExists = redis.opsForHash().hasKey(redisKey, "any_field");
            if (Boolean.TRUE.equals(hashExists)) {
                log.debug("Evento {} ya existe en Redis como hash, saltando inicialización", eventoId);
                return true;
            }
            
            // 3. Consultar datos del evento desde Cátedra
            ...
            // 4. Extraer dimensiones de la sala
            Integer filas = null;
            ...
            String imagen = eventoData.get("imagen") != null ? eventoData.get("imagen").toString() : null;
            // 5. Generar matriz completa de asientos como "Libre"
            ...
            // 6. Crear estructura completa del evento
            ...
            eventoCompleto.put("imagen", imagen); 

            // 7. Persistir en Redis como JSON
